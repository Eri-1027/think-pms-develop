...
export default {
  ...
  methods: {
    submitHandler () {
      const activeClassList = this.$refs.activate.classList
      if (!activeClassList.contains('loading')) {
        this.$emit('submit')
        if (this.loginStatus === 'loading') {
          activeClassList.add('loading')
        }

        if (this.loginStatus === 'success') {
          activeClassList.add('done')
        }
        if (this.loginStatus === '') {
          activeClassList.remove('loading')
          activeClassList.remove('done')
        }
      }
    }
  }
    ...
    &.done {
      ...
      span {
        ...
        svg {
          ...
          &:nth-child(3) {
            transition: transform .4s cubic-bezier(.175, .885, .32, 1.275) .3s, opacity .4s ease .25s;
            transform: scale(1);
